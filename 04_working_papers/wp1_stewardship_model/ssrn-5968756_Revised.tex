...
Chronic homelessness in the United States persists because prevailing interventions address housing placement without resolving the physiological preconditions. They fail to build the relational foundations. They ignore the economic stability required for durable tenure. This paper establishes the foundational theoretical framework for Systemic Dignity Infrastructure (SDI), a compound architecture that identifies three systemically distinct layers of intervention including Material Dignity Infrastructure (MDI) addressing biological stabilization. It includes Relational Dignity Infrastructure (RDI) addressing the social environment. It includes Economic Dignity Infrastructure (EDI) addressing the compound Return Deficit. Drawing on the capabilities approach, comparative international evidence from Finland, and domestic program-level outcomes, the paper evaluates the fiscal drain of emergency shelter. It evaluates the legal barriers. It analyzes the functional failures of concentration-based responses. It proposes a centralized tower architecture organized around Dunbar-scaled residential pods. It requires continuous relational presence through Pod Stewards. It deploys CalAIM-funded care coordination within the clinical workforce. Cost projections indicate an annual Efficiency Surplus per tower through cross-system cost avoidance, subject to independent financial review. Seven binary verification metrics constitute a Singular Prototype Threshold that must pass before network expansion proceeds. This paper serves as the first in a five-paper series; subsequent papers specify surplus capacity activation. They detail the Los Angeles implementation pipeline. They define relational infrastructure and economic reintegration mechanisms.
...
Martha Nussbaum translated Sen's framework into practice by proposing a list of central human capabilities. Her list includes bodily health with adequate shelter serving as a core component \citep{nussbaum_2011}. Housing functions as a foundational capability enabling the pursuit of other valued states It does not function as a basic commodity. Decent, affordable, and secure housing provides the essential base for what \citet{saunders_1989} terms ontological security. Individuals require this security to engage in economic activity, social relationships, and political participation.

Recent scholarship extends the capabilities approach explicitly to housing policy. Researchers argue that policy should focus on expanding housing-related capabilities It does not provide shelter as an isolated resource \citep{cambridge_2019}. This framework considers both access to physical structures and the broader freedoms that housing enables. These freedoms include protection from chronic stress and health insecurity. They include the capacity to participate in community life. They provide the stability to pursue employment and education without the constant disruption of housing instability.
...
The 86\% retention rate reported for Community First! Village in Section 3.2.2 is drawn from a Substack publication citing the organization's own program data. The 92\% one-year housing retention rate reported for Grace Marketplace in Section 3.3.1 is drawn from CauseIQ, a nonprofit database that aggregates self-reported organizational metrics. Neither figure has been independently replicated or published in peer-reviewed literature as of the date of this paper. Both figures are retained because they are the only available program-level data for these sites and are consistent with outcomes reported by comparable high-performing programs. Readers should treat them as indicative They are not independently verified benchmarks. The paper's core cost and savings projections in Section 5 are not dependent on either figure.
...
\begin{itemize}
    \item \textbf{Hygiene Infrastructure}: Private showers, laundry, and restrooms functioning continuously.
    ...
    \item \textbf{Harm Reduction}: Supervised consumption and naloxone distribution.
...
Above the Phase Zero clinical floors, the tower organizes residential space into 18 pods of approximately 116 residents each. The 116-resident pod size derives from the Dunbar community ceiling of 150 minus 34 permanent on-site staff, following Dunbar's social brain hypothesis \citep{dunbar_1992}. This hypothesis identifies the cognitive limit on the number of stable social relationships a human can maintain. Every resident within a pod is known by name and face to every other resident and to the Pod Steward. This relational saturation produces the community of recognition that institutional housing historically fails to create.

Each pod contains:

\begin{itemize}
    \item \textbf{Private Residential Units}: Individual rooms with STC 65 acoustic isolation equivalent to luxury hotel construction provide sovereign private space within a communal structure. Acoustic isolation functions as a clinical design parameter It is not an amenity. It eliminates the hypervigilance cues that prevent restorative sleep in congregate settings.
    ...
    \item \textbf{Biophilic Infrastructure Nodes}: Living walls, natural light wells, and planted common areas induce neurological downregulation \citep{porges_2011}. These nodes function as clinical infrastructure They are not aesthetic features. Polyvagal theory indicates that environmental cues signaling safety produce measurable reductions in sympathetic nervous system activation.
...
\begin{itemize}
    ...
    \item \textbf{Pipeline D}: Voluntary nomadic individuals who decline residential placement. The framework respects this autonomy while maintaining unconditional Open Resource Center access. Pipeline D individuals register as unique service recipients They are not program failures. They receive service through ground-floor infrastructure without residential enrollment.
\end{itemize}
...
The tower produces revenue through three primary channels derived from sovereign state and federal funding mechanisms It avoids volatile municipal grant cycles:
...
The California Advancing and Innovating Medi-Cal program provides a partial funding architecture for eligible care coordination roles \citep{dhcs_calaim_2024}. Enhanced Care Management and In Lieu of Services function as Medi-Cal billing categories that allow managed care plans to fund specific housing-related clinical services at federal matching rates. ECM billing applies to defined care coordination services including assessment, care planning, and referral functions. ILOS Community Supports covers a defined menu of services that each managed care plan individually elects to offer. Reimbursement rates are negotiated per managed care plan They are not set by a binding statewide schedule. This revenue applies exclusively to eligible peer support, case management, and housing navigation roles. Non-clinical operating staff including security, kitchen, facilities, and building maintenance have no pathway to Medicaid reimbursement under ECM or ILOS and constitute hard facility operating costs. The revenue projection assumes approximately seventy-five percent Medi-Cal enrollment among the target population, consistent with published enrollment estimates for the chronically unsheltered. The CalAIM revenue figures in this table are preliminary and subject to independent financial review.

\subsection{The Efficiency Surplus}

The Efficiency Surplus represents the difference between the fully-loaded MDI infrastructure cost, including amortized capital debt service, and the cross-system emergency costs that the tower's population would otherwise produce. Empirical data from high-acuity homeless cohorts consistently documents annual per-person emergency costs ranging from \$30,000 to \$50,000 across healthcare, criminal justice, and crisis services \citep{culhane_metraux_hadley_2002, larimer_2009}.

\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{@{}lXXX@{}}
\toprule
\textbf{Fiscal Category} & \textbf{Per-Person Annual} & \textbf{Prototype Annual (348 residents)} & \textbf{Full Build-Out Annual (2,000 residents)} \\
\midrule
\textbf{Cross-System Emergency Costs (Baseline)} & \$30,000--\$50,000 & \$10.4--\$17.4 Million & \$60--\$100 Million \\
\textbf{Fully-Loaded MDI Cost} & \$24,000--\$29,000 & \$8.4--\$10.1 Million & \$48--\$58 Million \\
\textbf{Revenue from CalAIM + Enterprise} & --- & \$5.2--\$7.7 Million & \$30--\$44 Million \\
\textbf{Net Efficiency Surplus} & --- & \textbf{\$0.35--\$9.05 Million} & \textbf{\$2--\$52 Million} \\
\bottomrule
\end{tabularx}
\end{table}

The Efficiency Surplus at the 348-resident Singular Prototype scale ranges from approximately \$0.35 million to \$9.05 million annually, reflecting the difference between the fully-loaded MDI infrastructure cost, inclusive of amortized capital debt service, and avoided emergency system costs, net of CalAIM and enterprise revenue. At full network build-out (2,000 residents), this range expands to approximately \$2 million to \$52 million annually. This expansion reflects the larger population base, not additional per-resident efficiency. Fixed staffing cost per resident does not improve beyond the three-pod scale, since the Mega-Tower Segregation Requirement mandates independent fixed staffing for each three-pod sector. It does not share fixed costs across the full tower. This surplus functions as a measure of fiscal displacement. It is not a profit center. Public dollars currently spent on emergency responses that produce no durable outcome are redirected toward infrastructure that produces measurable stabilization, retention, and transition metrics. Both ranges are provisional pending independent financial review.
...
\textbf{MDI Tower (Fully-Loaded, 348-Resident Prototype)} & \textbf{\$21,000--\$24,000} & \textbf{\$3,000--\$5,000} & \textbf{\$24,000--\$29,000} & \textbf{Target 85\%+} \\
\bottomrule
\end{tabularx}
\end{table}

\textit{The MDI Tower figures reflect the 348-resident Singular Prototype basis established elsewhere in this series, inclusive of amortized capital debt service; see the Efficiency Surplus table above for the corresponding tower-level calculation at both prototype and full-build-out scale. This figure is provisional pending independent financial review.}
...
The only exceptions to voluntary residency remain the existing legal mechanisms of Lanterman-Petris-Short Conservatorship for individuals who remain gravely disabled and unable to provide for basic needs and the Community Assistance, Recovery, and Empowerment Court for individuals with schizophrenia spectrum disorders who meet specific statutory criteria. These function as external legal processes with independent judicial oversight They are not internal facility policies.

\subsection{The Singular Prototype Threshold}

The framework includes an explicit falsifiability mechanism. Seven binary verification metrics constitute the Singular Prototype Threshold. All seven must pass at the Singular Prototype, a three-pod, 348-resident section deployed within the proposed One California Plaza site in Los Angeles, before network expansion to the site's full 2,000-unit capacity or to additional towers receives authorization:
...
The primary existential risk to the framework is that the tower becomes a terminal destination It fails as a transitional stabilization node. If residents remain indefinitely without progressing toward independent housing, the tower reproduces the warehousing dynamic that the architecture explicitly rejects.

\textbf{Structural Safeguard}: The Tenancy Bridge Guarantee specified in Working Paper 5 creates a financial mechanism that routes cooperative enterprise revenue into external tenancy support. Explicit residency timelines targeting eighteen to thirty-six months, transition accountability metrics, and discharge planning beginning at ninety days post-enrollment prevent institutional inertia. The Singular Prototype Threshold includes retention rate measurement at twenty-four months specifically to verify that residents transition out and do not accumulate indefinitely \citep{dibella_2026_wp5}.
...
\textbf{Structural Safeguard}: The architecture imposes fixed term limits for all cooperative board positions with mandatory rotation preventing consecutive terms. Bylaws receive adoption before facility activation They are not negotiated under acute pressure. An independent ombudsperson maintains authority to investigate governance complaints. The framework tracks governance participation as a metric feeding into the Singular Prototype Threshold.
...
\textbf{Structural Safeguard}: The framework reverses this sequence. Phase Zero clinical infrastructure, Assertive Community Treatment team staffing, and the Medicaid billing apparatus must achieve functional activation before the first resident enrolls. The tower conversion proceeds in parallel with clinical workforce development It does not proceed sequentially. Capital flows into a building only when the clinical infrastructure is ready to receive residents. The Singular Prototype Threshold introduced in Section 6.5 ensures that expansion cannot proceed until the prototype demonstrates functional readiness.
...
\begin{itemize}
    ...
    \item \textbf{Program-Related Investments}: This mechanism deploys foundation capital as investment It is not a grant. Investments from major foundations including Ford, MacArthur, and Kresge can bridge the gap between catalytic equity and senior debt.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Assertive Community Treatment Workforce}: Assertive Community Treatment teams provide the clinical backbone. Team members including psychiatrists, nurses, social workers, and peer specialists remain embedded in the tower They do not function from external offices. This centralized model eliminates the travel overhead that degrades clinical contact time in scattered-site models.
...
Third, it introduces the centralized tower architecture as the proposed implementation mechanism. This model requires the adaptive reuse of distressed commercial real estate organized around Dunbar-scaled residential pods, Phase Zero metabolic stabilization, and Medicaid-funded clinical workforce layers. This architecture replaces the concentration paradigm through internal spatial design that achieves relational density within a centralized facility It avoids suburban dispersion.
...
The Singular Prototype Threshold commits this framework to empirical verification before expansion. If the seven binary metrics pass at the prototype site, the architecture achieves validation and network expansion proceeds. If any metric fails, the failure mode undergoes diagnosis and the framework undergoes revision. This framework functions as an infrastructure specification premised on falsifiability It is not a policy proposal premised on optimism.
